...
public class NonOverlappingIntervals {
    ...
    public static int eraseOverlapIntervals(int[][] intervals) {
        ...
        List<Bound> intervalsList = new ArrayList<>();
        int ans = 0;
        for (int[] row : intervals){
            intervalsList.add(new Bound(row[0], row[1]));
        }

        Collections.sort(intervalsList, (Bound o2, Bound o1) ->{
                if (o2.lb == o1.lb) {
                    return o2.ub-o1.ub;
                }
                return o2.lb-o1.lb;
            });
        Bound prev = intervalsList.get(0);
        for (int i = 1; i < intervalsList.size(); i++) {
            Bound curr = intervalsList.get(i);
            if (prev.ub > curr.lb) {
                ans++;
                if (curr.ub < prev.ub)
                    prev = curr;
